"""
FPICRNNs for solving tfrde (high-frequency)
"""
...
def load_checkpoint(model, optimizer, scheduler, save_path):
    if save_path is None or (not os.path.isfile(save_path)):
        print(f"[WARN] checkpoint not found, skip loading: {save_path}")
...
def train(model, u0, initial_state, n_iters, learning_rate, save_path, pre_model_save_path=None):
    ...
    if pre_model_save_path is not None and os.path.isfile(pre_model_save_path):
        ...
        if pre_model_save_path is not None:
            print(f"[WARN] pre_model_save_path set but not found: {pre_model_save_path}")
...
if __name__ == '__main__':
    ...
    # 1) 需要训练：把 n_iters_adam 改成 2000，pre_model_save_path=None
    # 2) 只推理：n_iters_adam=0，并把 pre_model_save_path 指向你的权重
    n_iters_adam = 2000
    pre_model_save_path = None
    # save paths
    save_dir = f'./tfrde_seed_{SEED}'
    os.makedirs(save_dir, exist_ok=True)
    model_save_path = os.path.join(save_dir, 'fpicrnns_tfrde_highfreq.pt')
    ...
    # train
    logs = train(model, u0, initial_state, n_iters_adam, lr_adam, model_save_path, pre_model_save_path=pre_model_save_path)
    ...
    # 优先用预训练，其次用训练保存的；
    ckpt_candidates = [pre_model_save_path, model_save_path]
    ckpt_path = next((p for p in ckpt_candidates if p is not None and os.path.isfile(p)), None)

    model_test, _, _ = load_checkpoint(model_test, optimizer=None, scheduler=None, save_path=ckpt_path)
    ...
    for i in range(min(NN, pred.shape[0], truth.shape[0])):
